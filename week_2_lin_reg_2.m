...
in_sample_fraction_incorrect = zeros( 1000 , 1 ) ;
...
% linear regression on training_data_set and y
...
% now apply this w to classify
y_lin_est = sign( [ ones( N , 1 ) training_data_set ] * w ) ;

in_sample_fraction_incorrect(ii) = sum( y ~= y_lin_est ) / N ;

% generate the out_of_sample data
x_axis = ( rand( 1000 , 1 ) .- 0.5 ) .* 2 ; y_axis = ( rand( 1000 , 1 ) .- 0.5 ) .* 2 ;
...
% now apply this w to classify
y_lin_est = sign( [ ones( 1000 , 1 ) x_axis y_axis ] * w ) ;

out_of_sample_fraction_incorrect(ii) = sum( y ~= y_lin_est ) / 1000 ;

end

in_sample_error = mean( in_sample_fraction_incorrect )
out_of_sample_error = mean( out_of_sample_fraction_incorrect )
